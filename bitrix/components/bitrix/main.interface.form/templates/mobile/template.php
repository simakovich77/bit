...
CUtil::InitJSCore(array('ajax', 'date', "mobile_fastclick"));
$userUrl = str_replace("//", "/", "/".SITE_DIR."mobile/users/?user_id=#ID#");
$groupUrl = str_replace("//", "/", "/".SITE_DIR."mobile/log/?group_id=#ID#");

?>
<div class="mobile-grid-form mobile-grid <?if ($arParams["RESTRICTED_MODE"]) echo "mobile-grid-modified";?>">
						...
						else
						{
							...
							switch($field["type"])
							{
								...
								case 'disk':
									...
									$className = "file";
									ob_start();
									$APPLICATION->IncludeComponent("bitrix:system.field.".($field["type"] == "disk" ? "edit" : "view"), "disk_file",
										array(
											"arUserField" => $val,
											"MOBILE" => "Y",
